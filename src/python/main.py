...
class PointCloudProcessor:
    def __init__(self):
        ...
        # 新增速度相关变量
        self.last_transformation = np.eye(4)  # 上一帧的变换矩阵
        self.last_time = rospy.Time.now()  # 上一帧的时间戳
        self.velocity = np.zeros(3)  # 初始化速度为零
    ...
    def process_pointcloud(self, pc_msg):
        ...
        dt = (current_time - self.last_time).to_sec()  # 时间间隔
        ...
        initial_transformation[:3, 3] = self.velocity * dt

        # 4) GICP测试
        transformation_icp = self.gicp.register(pointcloud_data_to_001, self.map_points, initial_transformation)
        ...
        # 6) 计算速度
        displacement = transformation_icp[:3, 3]
        self.velocity = displacement / dt if dt > 0 else np.zeros(3)

        # 7) 如果处理的帧数超过10帧，关闭程序
        if self.frame_count > 20:
            print(f"处理的帧数超过 {self.frame_count} 帧，关闭程序")
            rospy.signal_shutdown(f"处理的帧数超过 {self.frame_count} 帧,程序正常关闭")
            if hasattr(self, 'rosbag_process'):
                self.rosbag_process.terminate()
                self.rosbag_process.wait()
        else:
            print(f"正在进行第 {self.frame_count} 帧的数据处理")
